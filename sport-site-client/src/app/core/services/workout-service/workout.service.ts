...
export class WorkoutService {
  ...
  updateWorkout(id: string, payload: Partial<Workout>): Observable<Workout> {
    const url = `${this.baseUrl}/userworkout/${id}`;
    return this.http.patch<Workout>(url, payload, { headers: this.headers, withCredentials: true });
  }
  ...
  createWorkout(payload: Workout): Observable<Workout> {
    ...
    return this.http.post<Workout>(url, payload, { headers: this.headers, withCredentials: true });
  }
